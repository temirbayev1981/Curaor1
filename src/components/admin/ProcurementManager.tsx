...
export function ProcurementManager() {
  ...
  const loadData = useCallback(async () => {
    try {
      ...
      setError(t('admin.procurement.fetchError'));
    } finally {
      setLoading(false);
    }
  }, [t]);
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (!cancelled) setError(t('admin.procurement.fetchError'));
      } finally {
  ...
  async function generateLowStock() {
    ...
    if (!res.ok || json.error) {
      setError(json.error?.message ?? t('admin.procurement.generateError'));
    } else {
      await loadData();
    }
    setGenerating(false);
        ...
        <div className="flex flex-wrap items-end gap-4">
          <div className="min-w-[200px] flex-1">
            <label className="mb-1 block text-xs text-zinc-500">
              {t('admin.procurement.linkBooking')}
            </label>
            <select
              value={bookingId}
            ...
            >
              <option value="">{t('admin.procurement.noBooking')}</option>
              {bookings.map((b) => (
          ...
          <Button loading={generating} onClick={generateLowStock}>
            ...
            {t('admin.procurement.generateLowStock')}
          </Button>
        </div>
              ...
              <div className="text-right">
                ...
                <div className="mt-2 flex gap-2">
                  {(['draft', 'ordered', 'received'] as const).map((status) => (
                    ...
                    >
                      {t(`admin.procurement.status.${status}`)}
                    </Button>
                  ))}
                </div>
              </div>
